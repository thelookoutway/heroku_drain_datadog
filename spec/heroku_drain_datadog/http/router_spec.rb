      ...
      context "and drain token header" do
        ...
        context "router logs" do
          ...
          it "sends a histogram for connect" do
            expect(output[0]).to eq("heroku.router.connect:0.0|h|#dyno:web.1,dynotype:web,method:GET,status:304")
          end

          it "sends a histogram for service" do
            expect(output[1]).to eq("heroku.router.service:2.0|h|#dyno:web.1,dynotype:web,method:GET,status:304")
          end
        ...
        context "dyno memory logs" do
          ...
          it "sends a gauge for memory_cache" do
            expect(output[0]).to eq("heroku.dyno.memory_cache:3.63|g|#dyno:web.1,dynotype:web")
          end

          it "sends a gauge for memory_quota" do
            expect(output[1]).to eq("heroku.dyno.memory_quota:512.0|g|#dyno:web.1,dynotype:web")
          end

          it "sends a gauge for memory_rss" do
            expect(output[2]).to eq("heroku.dyno.memory_rss:139.79|g|#dyno:web.1,dynotype:web")
          end

          it "sends a gauge for memory_swap" do
            expect(output[3]).to eq("heroku.dyno.memory_swap:11.43|g|#dyno:web.1,dynotype:web")
          end

          it "sends a gauge for memory_quota" do
            expect(output[4]).to eq("heroku.dyno.memory_total:154.85|g|#dyno:web.1,dynotype:web")
          end
        ...
        context "and drain env var" do
          it "can be a single tag" do
            with_env("DRAIN_TAGS_FOR_abc123", "service:myapp") do
              post "/logs", %q{338 <158>1 2016-08-20T02:15:10.862264+00:00 host heroku router - at=info method=GET path="/assets/admin-62f13e9f7cb78a2b3e436feaedd07fd67b74cce818f3bb7cfdab1e1c05dc2f89.css" host=app.fivegoodfriends.com.au request_id=bef7f609-eceb-4684-90ce-c249e6843112 fwd="58.6.203.42,54.239.202.42" dyno=web.1 connect=0ms service=2ms status=304 bytes=112}
              expect(output[0]).to eq("heroku.router.connect:0.0|h|#service:myapp,dyno:web.1,dynotype:web,method:GET,status:304")
            end
          end
          ...
          it "can be many tags" do
            with_env("DRAIN_TAGS_FOR_abc123", "env:production,service:myapp") do
              post "/logs", %q{338 <158>1 2016-08-20T02:15:10.862264+00:00 host heroku router - at=info method=GET path="/assets/admin-62f13e9f7cb78a2b3e436feaedd07fd67b74cce818f3bb7cfdab1e1c05dc2f89.css" host=app.fivegoodfriends.com.au request_id=bef7f609-eceb-4684-90ce-c249e6843112 fwd="58.6.203.42,54.239.202.42" dyno=web.1 connect=0ms service=2ms status=304 bytes=112}
              expect(output[0]).to eq("heroku.router.connect:0.0|h|#env:production,service:myapp,dyno:web.1,dynotype:web,method:GET,status:304")
            end
          end
